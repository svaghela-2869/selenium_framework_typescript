...
describe(
  __filename.split("selenium_framework_typescript/")[1],
  async function () {
    ...
    for (let i = 0; i < 10; i++) {
      it("CODEZEROES - DUMP", async function () {
        ...
        await globalConfig.driver
          .findElement(By.xpath("//select[@id='contact-choice']"))
          .sendKeys(choiceAvailable[randomChoice]);
        await globalConfig.driver
          .findElement(By.xpath("//input[@id='contact-name']"))
          .sendKeys(dumpName);
        await globalConfig.driver
          .findElement(By.xpath("//input[@id='contact-email']"))
          .sendKeys(dumpName + "." + dumpNumber + "@codezeroes.dump.com");
        await globalConfig.driver
          .findElement(By.xpath("//input[@id='contact-phone']"))
          .sendKeys(randomMobile);
        await globalConfig.driver
          .findElement(By.xpath("//textarea[@id='contact-message']"))
          .sendKeys(dumpDescription);

        await reporter.pass("Data entered in fields.", true);
        await globalConfig.driver
          .findElement(By.xpath("//input[@type='submit']"))
          .click();
        await uihelper.waitForElementToBePresentOnUI(
          "//input[@value='Sent']",
          10
        );
