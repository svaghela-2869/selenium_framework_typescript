...
before(async function () {
  ...
  let apiCallsToMake = await utils_common.get_all_api_calls(globalConfig.spec.csv);
  values_to_check = [];
  ...
  describe(globalConfig.spec.name, async function () {
    ...
    if (values_to_check.length > 0) {
      for (let i = 0; i < values_to_check.length; i++) {
        let step = values_to_check[i];
        (step.startsWith("skip") ? xit : it)("verify_read_only_text", async function () {
          await uihelper.verify_read_only_text(values_to_check[i]);
